...
#define SHARED_MEMORY_NAME "/exercicio3"
#define SIZE 100000
...
int main(int argc, char *argv[]){
    ...
    int fd, size = SIZE*sizeof(test_t);
    test_t *addr;
    ...
    */
    addr = (test_t*)mmap(NULL, size, PROT_READ, MAP_SHARED, fd, 0);
    if(addr == MAP_FAILED){
        perror("mmap");
        exit(EXIT_FAILURE);
    }
    ...
    printf("\n[READER] Timer to read starting now...");
    clock_t ini = clock();
    for(int i=0;i<SIZE;i++){
        local_array[i].num = addr[i].num;
        strcpy(local_array[i].str, addr[i].str);
    }
    clock_t end = clock();
    double time = (double)(end - ini) / CLOCKS_PER_SEC;
    printf("\n\n[READER] Read operation finished with %f!\n", time);

    printf("\n\n[READER] First: %d|%s\n[READER] Middle: %d|%s\n[READER] Last: %d|%s\n", local_array[0].num, local_array[0].str,
     local_array[SIZE/2].num, local_array[SIZE/2].str,
      local_array[SIZE-1].num, local_array[SIZE-1].str);
