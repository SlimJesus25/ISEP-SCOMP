...
#define READER "/home/ricardo/Documents/SCOMP/scomp2023_2de_1210828/Sprint2/Shared Memory/ex3/shared_memory_reader"
...
int main(){
    ...
    */
    addr = (test_t*)mmap(NULL, size, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0);
    if(addr == MAP_FAILED){
        perror("mmap");
        exit(EXIT_FAILURE);
    }
    printf("\n[WRITER] Filling test_t array with data...\n");
    timer_t ini = clock();
    fill_test_t_array(addr);
    timer_t end = clock();
    double time = (double)(end-ini)/CLOCKS_PER_SEC;
    printf("\n[WRITER] Write done!\n[WRITER] Operation took %f seconds", time);
