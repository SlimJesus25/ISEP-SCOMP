...
#define SHARED_MEMORY_NAME "/exercicio4"
#define SIZE 100
#define A 65
#define Z 90

void generate_random_chars(char* array, int size){
    srand(time(NULL));
...
int main(){
    ...
    */
    fd = shm_open(SHARED_MEMORY_NAME, O_CREAT|O_TRUNC|O_RDWR, S_IRUSR|S_IWUSR);
    if(fd < 0){
        perror("shm_open");
        exit(EXIT_FAILURE);
    }
    ...
    */
    ...
    generate_random_chars(addr, size);
