...
#define SHARED_MEMORY_NAME "/exercicio2"
#define SIZE 2
...
int main()
{
    ...
    */
    addr = (int *)mmap(NULL, SIZE * sizeof(int), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    if (addr == MAP_FAILED)
    {
        perror("mmap");
        exit(EXIT_FAILURE);
    ...
    // Inicializando os valores iniciais dos 2 inteiros na memória partilhada.
    *(addr) = 10000;
    *(addr + 1) = 500;
    ...
    if (p == 0)
    {
        printf("\n[CHILDREN] First value: %d\nSecond value: %d\n", *(addr), *(addr + 1));
        exit(EXIT_SUCCESS);
    }
    wait(NULL);
    printf("\n[FATHER] First value: %d\nSecond value: %d\n", *(addr), *(addr + 1));
